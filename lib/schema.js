(function() {
  var URL, b, buf2str, checkAnnounce, checkAnnounceList, checkBuffer, checkCreationDate, checkTorrent, checkmd5sum, crypto, getInfoHash, isInteger, isURL, validate;
  ...
  checkAnnounce = function(torrent) {
    ...
    if (!isURL(torrent.announce)) return '`announce` is not a URL';
    return null;
  };
  ...
  checkAnnounceList = function(list) {
    ...
    if (list) {
      ...
      for (_i = 0, _len = list.length; _i < _len; _i++) {
        item = list[_i];
        if (!Array.isArray(item)) return '`announce-list` is not a list of lists';
        for (i = 0, _len2 = item.length; i < _len2; i++) {
          ann = item[i];
          if (!Buffer.isBuffer(ann)) {
            return 'Field in `announce-list` is not a buffer';
          }
          if (!isURL(item[i])) return 'Item in `announce-list` list is not a URL';
        }
      }
    }
    return null;
  ...
  checkCreationDate = function(date) {
    if ((date != null) && !isInteger(date)) return '`date` is not an integer';
    return null;
  };
  ...
  buf2str = function(torrent, path) {
    var key, val, _i, _len, _ref;
    _ref = Object.keys(torrent);
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      key = _ref[_i];
      val = torrent[key];
  ...
  validate = function(torrent, callback) {
    var err;
